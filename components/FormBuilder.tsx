...
const FormBuilder: React.FC<JobApplicationFormProps> = ({ sharedData, setSharedData }) => {
    ...
    const editFieldRequired = (event: React.ChangeEvent, index: number) => {
        ...
        setSharedData(sharedData.map((field, i) => i === index ? { ...field, required: checked } : field))
    }

    const moveFieldUp = (index: number) => {
        if (index === 0) return;
        const newSharedData = [...sharedData];
        const temp = newSharedData[index];
        newSharedData[index] = newSharedData[index - 1];
        newSharedData[index - 1] = temp;
        setSharedData(newSharedData);
    }

    const moveFieldDown = (index: number) => {
        if (index === sharedData.length - 1) return;
        const newSharedData = [...sharedData];
        const temp = newSharedData[index];
        newSharedData[index] = newSharedData[index + 1];
        newSharedData[index + 1] = temp;
    ...
    const updateOption = (index: number, optionIndex: number, event: React.ChangeEvent) => {
        const { value } = event.target;
        setSharedData(sharedData.map((field, i) => i === index ? { ...field, options: field.options.map((option, j) => j === optionIndex ? value : option) } : field))
    }

    const addOption = (index: number) => {
        setSharedData(sharedData.map((field, i) => i === index ? { ...field, options: [...field.options, ''] } : field))
    }

    const deleteOption = (index: number, optionIndex: number) => {
        setSharedData(sharedData.map((field, i) => i === index ? { ...field, options: field.options.filter((_, j) => j !== optionIndex) } : field))
    }
